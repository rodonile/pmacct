		...
		int *has_templates)
{
  ...
  if (fid == 0 || fid == 2) { /* template: 0 NetFlow v9, 2 IPFIX */ 
	...
	case PM_FTYPE_IPV4:
	  ...
	  if (config.bgp_daemon_to_xflow_agent_map) BTA_find_id((struct id_table *)pptrs->bta_table, pptrs, &pptrs->bta, &pptrs->bta2);
	  if (config.nfacctd_flow_to_rd_map) NF_find_id((struct id_table *)pptrs->bitr_table, pptrs, &pptrs->bitr, NULL);
          Log(LOG_INFO, "INFO ( %s/core ): NF_find_id - RD from bitr=%d\n", config.name, ((rd_t *)&pptrs->bitr)->val); //TMP
	...
	case PM_FTYPE_IPV6:
	  ...
	  if (config.nfacctd_flow_to_rd_map) NF_find_id((struct id_table *)pptrs->bitr_table, &pptrsv->v6, &pptrsv->v6.bitr, NULL);
          Log(LOG_INFO, "INFO ( %s/core ): NF_find_id - RD from bitr=%d\n", config.name, ((rd_t *)&pptrs->bitr)->val); //TMP //TODO: print only 16bits of val to see it for rd_ip types¨!!!
...
void NF_mpls_vpn_rd_from_options(struct packet_ptrs *pptrs)
{
  ...
  case 9:
    ...
    Log(LOG_INFO, "INFO ( %s/core ): NF_mpls_vpn_rd_from_options - RD from bitr=%d\n", config.name, ((rd_t *)&pptrs->bitr)->val); //TMP
    if (!((rd_t *)&pptrs->bitr)->val) { /* RD was not set with flow_to_rd_map or from IPFIX/NFv9 data */

      Log(LOG_INFO, "INFO ( %s/core ): NF_mpls_vpn_rd_from_options - Looking for RD from option data...\n", config.name); //TMP
    ...
    Log(LOG_INFO, "INFO ( %s/core ): NF_mpls_vpn_rd_from_options [end] - RD from bitr=%d\n", config.name, ((rd_t *)&pptrs->bitr)->val); //TMP
...
void NF_mpls_vpn_rd_from_ie90(struct packet_ptrs *pptrs)
{
  ...
  case 9:
    if (tpl->fld[NF9_MPLS_VPN_RD].count) {
      Log(LOG_INFO, "INFO ( %s/core ): NF_mpls_vpn_rd_from_ie90 - RD from bitr=%d\n", config.name, ((rd_t *)&pptrs->bitr)->val); //TMP
      if (!((rd_t *)&pptrs->bitr)->val) { /* RD was not set with flow_to_rd_map */

        Log(LOG_INFO, "INFO ( %s/core ): NF_mpls_vpn_rd_from_ie90 - Looking for RD from option data...\n", config.name); //TMP
